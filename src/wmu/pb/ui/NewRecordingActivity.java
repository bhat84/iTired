 ...
 * Modified by @author Praveen Bhat  
 * Modified appendDataToGraphs function  
 */
		...
		public void handleMessage(Message msg) {
			...
			case BiopluxService.MSG_DATA: {
				...
				appendDataToGraphs(
						...
						msg.getData().getDouble(BiopluxService.KEY_FRAME_DATA));
				break;
			}
	...
	void appendDataToGraphs(double xValue, double data) {
		if(!serviceError){
			//for (int i = 0; i < graphs.length; i++) {
				...
				graphs[0].getSerie().appendData(new GraphViewData(xValue,data), goToEnd, maxDataCount);
			//graph.getSerie().appendData(new GraphViewData(xValue,data), goToEnd, maxDataCount);
